...
public class DAO {
	...
	private static final String[] TASK_FIELDS = { TASK_ID, TASK_NAME,
			TASK_ACTIVE, TASK_ORDERING };
	...
	private List<Task> getTasksWithConstraint(String constraint) {
		...
		// ordering the tasks
		Cursor cursor = db.query(TASK, TASK_FIELDS, constraint, null, null,
				null, TASK_NAME);
		cursor.moveToFirst();
	...
	public Task updateTask(Task task) {
		...
		List<Task> updated = getTasksWithConstraint(TASK_ID + "="
				+ task.getId() + "");
	...
	private List<Week> getWeeksWithConstraint(String constraint) {
		...
		Cursor cursor = db.query(WEEK, WEEK_FIELDS, constraint, null, null,
				null, WEEK_START);
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
	...
	public Week updateWeek(Week week) {
		...
		List<Week> updated = getWeeksWithConstraint(WEEK_ID + "="
				+ week.getId());
		return updated.get(0);
	}
	...
	private static final String[] EVENT_FIELDS = { EVENT_ID, EVENT_WEEK,
			EVENT_TIME, EVENT_TYPE, EVENT_TASK, EVENT_TEXT };
	private static final String[] MAX_EVENT_FIELDS = { EVENT_ID, EVENT_WEEK,
			"max(" + EVENT_TIME + ")", EVENT_TYPE, EVENT_TASK, EVENT_TEXT };
	...
	public List<Event> getEventsOnDay(DateTime day) {
		return getEventsWithConstraint(EVENT_TIME + " like \""
				+ DateTimeUtil.dateTimeToDateString(day) + "%\"");
	}

	public Event getLatestEvent() {
		List<Event> latestEvent = getEventsWithParameters(MAX_EVENT_FIELDS,
				null);
		// if latestEvent is empty, then there is no event in the database
		return latestEvent.isEmpty() ? null : latestEvent.get(0);
	}

	private List<Event> getEventsWithParameters(String[] fields,
			String constraint) {
		List<Event> ret = new ArrayList<Event>();
		Cursor cursor = db.query(EVENT, fields, constraint, null, null, null,
				EVENT_TIME);
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
	...
	public Event updateEvent(Event event) {
		...
		List<Event> updated = getEventsWithConstraint(EVENT_ID + "="
				+ event.getId());
		return updated.get(0);
	}
